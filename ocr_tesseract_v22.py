# -*- coding: utf-8 -*-
"""
Created on Tue Sep  1 20:50:29 2020
...
class imagePrep:
    ...
    def convert_rotate(self, path_pdf, img_format, img_dpi, pop_path, output_name, output_path):
        
        """Converts, rotates, and exports each page in the PDF to the desired output format, saving each new image in the specified 'output_path'"""
    
        # assign tesseract.exe file location for Python to read from
        pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
    
        # create 'paths'--will be used to contain the paths of images after being rotated and saved and the containing folder
        paths = []
    
        # use pdf2image's 'pdfinfo_from_path' function to obtain the total number of pages in the source file
        num_pages = pdf2image.pdfinfo_from_path(pdf_path = pdf_path, poppler_path = pop_path)["Pages"] + 1
        
        # create the folder 'converted_images' which will store all converted images
        converted_path = create_directory(output_path, "converted_images")
        
        # using 'num_pages' as a cap, iterate, convert and save images in multiples of 5 to avoid memory errors 
        # (this is a very memory intensive section of code, so splitting in chunks of 5 is absolutely necessary to avoid out of memory errors)
        for i in range(1, num_pages, 5):
            # use pdf2image's 'convert_from_path' function to convert the PDF file to the specified output format/parameters
            converted_images = pdf2image.convert_from_path(pdf_path = path_pdf, fmt = img_format, dpi = img_dpi
                                                           , poppler_path = pop_path, first_page = i
                                                           , last_page = i+4, output_folder = converted_path
                                                           , grayscale = True)
    
            print("\t\nPages " + str(i) + " to " + str(i+4) + " converted")
            
            page_num = i
            for img in converted_images:
                if page_num < num_pages:
                    # create a folder named after the resulting exported image (exported image name = name of the PDF file + page number of the corresponding image)
                    # will be saved in the 'output_path' directory
                    folder = create_directory(output_path, output_name + "_page" + str(page_num))
                    
                    # using 'folder', create the full path the output file will be saved in. the file name will be based on 'output_name' followed by the page number and the output file format
                    output_file = os.path.join(folder, output_name + "_page" + str(page_num) + "." + output_fmt)
                    
                    # use pytesseract's 'image_to_osd' function to detect any orientation shifts in the image and rotate the image to a readable orientation. save in 'output_file'
                    img.rotate(pytesseract.image_to_osd(img, output_type = pytesseract.Output.DICT)['orientation'], expand = True).save(output_file)
                    paths.append([folder, output_file])
                    
                    # delete the following variables and use garbage collector to free up memory
                    del folder, img
                    gc.collect()
                    
                    page_num += 1
                    
            # delete the iteration of 'converted_images' and 'output_file', again, to free up memory
            del output_file, converted_images
            gc.collect()
    ...
    def crop_image(self, output_name, output_path, img_path, template_type):
        ...
        if template_type == 1:
            ...
            del top, bounding_boxes, boxes_dict
            gc.collect()
...
class extractItemData():
    ...
    def bounding_boxes(self, img, kernel, vertical_lines, horizontal_lines, text_removed):
        ...
        del img_vh, thresh, bitxor, M, contours, hierarchy
        gc.collect()
    ...
    def extract_item_data(self, bitnot_img, row, column_count, final_boxes):
        ...
        del final_img, kernel, resizing, dilation, erosion, out, final_boxes, row, column_count
        gc.collect()
...
def extract(prep_instance, list_paths, page_num):
    
    pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
    
    print("\t\nPage Number:" + str(page_num))
    img_path = prep_instance.remove_skew(img_path = list_paths[1], save_path = list_paths[0], page_num = page_num)
    print(img_path)
    type_result = prep_instance.template_type(img_path)
    
    # because the wrong template type can be detectde ocassionally, use a while loop to cycle through template types if an error is encountered
    counter = 1
    while counter <= 2:
        ...
        try:
            ...
            if type_result == 1:
                category_mgr, vendor_num, bill_to_vendor, vendor_name, date_start, date_end, items, crop_left, crop_right, date_box = prep_instance.crop_image(list_paths[1], list_paths[0], img_path, type_result)
            elif type_result == 2:
                category_mgr, vendor_num, bill_to_vendor, vendor_name, items, crop_left, crop_right = prep_instance.crop_image(list_paths[1], list_paths[0], img_path, type_result)
...
if __name__ == "__main__":
    ...
    output_name = pdf_path.rsplit("/", 1)[-1].split(".")[0]
    # img_dpi should be at least 300 for accurate extraction of text -- higher dpi impacts performance negatively
    img_dpi = 300 

    start = time.time()
    
    instance_prep = imagePrep()
    print("\nCONVERTING IMAGES")
    paths_list = instance_prep.convert_rotate(pdf_path, output_fmt, img_dpi, pop_path, output_name, output_path)
    print("\nIMAGES SUCCESSFULLY CONVERTED")
    
    results = []
    # print("\nEXTRACT PROCESS STARTED")
    # for pg_num, paths in enumerate(paths_list):
    #     data_extracted = extract(instance_prep, paths, pg_num)
    #     results.append(data_extracted)
    #     del data_extracted
    #     gc.collect()
        
    # multiprocessing version
    iterable = [[instance_prep, paths, pg_num] for pg_num, paths in enumerate(paths_list)]
    p = multiprocessing.Pool(2)
    extract_results = p.starmap(extract, iterable)
